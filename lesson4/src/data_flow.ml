...
module type FlowAnalysis = sig
  ...
  val meet : workset -> workset -> workset

  val transfer : workset -> workset cfg_node -> workset
  ...
  let construct_cfg
    ...
    List.iter2
      begin fun (first, last) block_name ->
        let succ_nodes =
          Core.String.Map.find func.cfg block_name
...
module DataFlow (F : FlowAnalysis) = struct
  let perform_analysis (top : F.workset) (cfg : F.workset cfg_node list) : unit =
    let open F in
    ...
    | _ ->
      while not (Queue.is_empty worklist) do
        ...
        let in_set =
          ...
          |> List.fold_left meet top in
        cfg_node.prev_workset <- in_set;
        (* transfer function *)
        let workset' = transfer in_set cfg_node in
